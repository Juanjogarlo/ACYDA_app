...
if (isset($_POST['username'], $_POST['email'], $_POST['p'])) {
    ...
    $prep_stmt = "SELECT id FROM administradores WHERE email = ? LIMIT 1";
   ...
   // Verifica el correo electrónico existente.  
    ...
    $prep_stmt = "SELECT id FROM administradores WHERE username = ? LIMIT 1";
    $stmt = $mysqli->prepare($prep_stmt);
    ...
    if (empty($error_msg)) {
        ...
        if ($insert_stmt = $mysqli->prepare("INSERT INTO administradores (username, email, password, salt) VALUES (?, ?, ?, ?)")) {
